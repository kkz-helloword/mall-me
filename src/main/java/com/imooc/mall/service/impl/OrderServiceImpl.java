...
public class OrderServiceImpl implements IorderService {
    ...
    public ResponseVo<OrderVo> create(Integer uid, Integer shippingId) {
        ...
        Map<Integer, Product> map = productList.stream()
                .collect(Collectors.toMap(Product::getId, product -> product));//实现可以传入的product return product.getid，不过这里是方法引用
        List<OrderItem> orderItemList = new ArrayList<>();
        Long orderNo = generateOrderNo();

        for (Cart cart : cartList) {
            //根据productId查数据库
            Product product = map.get(cart.getProductId());//封装到map即一次性全部查，避免多次根据id查找
            if (product == null) {
        ...
        int rowForOrderItem = orderItemMapper.batchInsert(orderItemList);//批量写入。xml属性要写出来
    ...
    public ResponseVo<PageInfo> list(Integer uid, Integer pageNum, Integer pageSize) {
        ...
        Map<Long, List<OrderItem>> orderItemMap = orderItemList.stream()
                .collect(Collectors.groupingBy(OrderItem::getOrderNo));//知识点,一个订单号多个产品，不是一对一map
        ...
        for (Order order : orderList){
            OrderVo orderVo = buildOrderVo(order,
                    ...
                    shippingMap.get(order.getPostage()));
